...
void *web(void *data)
{
    ...
    fd = param->fd;
    hit = param->hit;
    ret = read(fd, buffer, BUFSIZE); /* read web request in one go *///->buffer
    if (ret == 0 || ret == -1)
    ...
    else
    {
        ...
        if ((file_fd = open(&buffer[5], O_RDONLY)) == -1) //start
        ...
        if(s->len==0)
        {
            ...
            while ((ret = read(file_fd, buffer, BUFSIZE)) > 0)    //->buffer
            {
                (void)write(fd, buffer, ret);                         //->fd  end
                Enfilelink(s,buffer,len,ret);
            }
        }
        else{
            (void)sprintf(buffer, "http/1.1 200 ok\nserver: nweb/%d.0\ncontent-length: %ld\nconnection: close\ncontent-type: %s\n\n", VERSION, len, fstr); /* header + a blank line */
            ...
            (void)write(fd, buffer, strlen(buffer));
            int d = s->deep;
            (void)write(fd, s->buffer, s->ret);
            if(d!=0){
                xnode *p = s->xnext;
                while(d--){
                    (void)write(fd, p->buffer, p->ret);
                    p = p->next;
                }
            }
            logger(FORBIDDEN, "succeed write from cache", "", 0);
        }
